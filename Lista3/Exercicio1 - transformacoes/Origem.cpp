#include <../../Lista3/Exercicio1 - transformacoes/Shader.h>
...
int main()
{
	...
	Shader shader("transformations.vs", "transformations.frag");
	...
	float tx = 0.2f, txScale = 1;
	bool right = true, left = false, small = false, big = true;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(tx, 0.0f, 0.0f));
		if (tx < 4.4 && right) {
			tx += 0.005;
			if (tx >= 4.4) {
				right = false;
		...
		//Matriz de modelo - Transformações no objeto
		model = glm::scale(model, glm::vec3(txScale, txScale, 1.0f));
